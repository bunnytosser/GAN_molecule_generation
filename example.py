...
if not os.path.exists(direcs):
    os.makedirs(direcs)
if not os.path.exists(filesaver):
    os.makedirs(filesaver)
def train_fetch_dict(i, steps, epoch, epochs, min_epochs, model, optimizer):
    a = [optimizer.train_step_D] if (i % n_critic != 0 and epoch>10) else [optimizer.train_step_G]
    ...
    if la <=1 :
        ...
        if i % n_critic == 0:
            ...
            feed_dict = {model.edges_labels: a,
                         ...
                         optimizer.la: la if epoch <=(2*epochs/3) else 0.8}
...
# optimizer
optimizer = GraphGANOptimizer(model, learning_rate=0.0005, feature_matching=f_mapping)

# session
session = tf.Session()
session.run(tf.global_variables_initializer())

train_writer = tf.summary.FileWriter(os.path.join('summaries','first'), session.graph)
# trainer
trainer = Trainer(model, optimizer, session,filesaver)
...
trainer.train(batch_dim=batch_dim,
              ...
              directory=direcs, # here users need to first create and then specify a folder where to save the model
