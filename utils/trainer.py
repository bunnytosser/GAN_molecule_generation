...
class Trainer:
    ...
    def __init__(self, model, optimizer, session,filesaver):
        self.model, self.optimizer, self.session, self.filesaver,self.print= model, optimizer, session,filesaver, defaultdict(list)
    ...
    def train(self, batch_dim, epochs, steps,
        ...
        if _eval_step is None:
            def _eval_step(epoch, epochs, min_epochs, model, optimizer, batch_dim, eval_batch, start_time,
                ...
                with open(self.filesaver+"/epoch"+str(epoch),"w") as f:
        ...
        if _test_step is None:
            def _test_step(model, optimizer, batch_dim, test_batch, start_time, _test_update):
               # self.load(directory)
                ...
                with open(self.filesaver+"/epoch999","w") as f:
                    for k,v in output.items():
                        f.write(str(k)+"\t"+str(v)+"\n")
                f.close()
