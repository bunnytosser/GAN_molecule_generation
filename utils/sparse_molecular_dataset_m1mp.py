...
class SparseMolecularDataset():
    ...
    def load(self, filename, subset=1):
        ...
        self.__len = self.train_count

    def save(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump(self.__dict__, f)

    def generate(self, filename, add_h=False, filters=lambda x: True, size=None, validation=0, test=0):
...
if __name__ == '__main__':
    ...
    data.generate('../data/m1mp_out.sdf', filters=lambda x: x.GetNumAtoms() <= 32)
    data.save('../data/m1mp_nodes.sparsedataset')
