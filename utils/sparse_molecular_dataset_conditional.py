...
class SparseMolecularDataset():
    ...
    def generate(self, filename,protlist,add_h=False, filters=lambda x: True, size=None, validation=0.1, test=0.1):
        self.log('Extracting {}..'.format(filename))

        if filename.endswith('.sdf'):
            self.data = [(i,mol) for (i,mol) in enumerate(Chem.SDMolSupplier(filename)) if mol is not None]
        if len(self.data)!=len(protlist):
            print("inconsistant lengths")
            return("incorrect length of data!")
        elif filename.endswith('.smi'):
            self.data = [Chem.MolFromSmiles(line) for line in open(filename, 'r').readlines()]
        molonly=[i[1] for i in self.data]
        molonly_H = list(map(Chem.AddHs, molonly)) if add_h else molonly
        self.data=[(i[0],j) for (i,j) in zip(self.data,molonly_H)]

        self.data = [(i,j) for (i,j) in self.data if j.GetNumAtoms()<=maxlenatoms]
        self.selected_ind=[i for (i,j) in self.data]
        self.proteins=[protlist[j] for j in self.selected_ind]
        self.data=[j for (i,j) in self.data]
    ...
    def _generate_encoders_decoders(self):
        ...
        self.log("atom_encoder:{}".format(self.atom_encoder_m))
        ...
        self.log("bond_encoder_m:{}".format(self.bond_encoder_m))
    ...
    def _generate_AX(self):
        ...
        protein_features=pd.read_csv("../cvae/protein_latent_vector.csv")
        data = []
        smiles = []
        data_S = []
        data_A = []
        ...
        for i,(mol,prot) in enumerate(zip(self.data,self.proteins)):
            ...
            if A is not None:
                ...
                data_D.append(D)
                data_F.append(self._genF(i,protein_features))
    ...
    def _genF(self, i,protein_features):

        features=np.array(protein_features[protein_features["Proteins"]==self.proteins[i]].iloc[:,1:-1])
        return(features)
...
if __name__ == '__main__':
    ...
    protlist=[]
    maxlenatoms=16
    with open("../data/proteins.txt") as f:
        for i in f:
            protlist.append(i.strip())
    data.generate('../data/pp_out.sdf',protlist, filters=lambda x: x.GetNumAtoms() <= maxlenatoms)
    data.save('../data/cvae16_nodes.sparsedataset')
