    ...
    if kl_weight <=1:
        ...
        if i % n_critic == 0:
            ...
            feed_dict = {model.edges_labels: a,
                         ...
                         model.training: True,
                         optimizer.la: la,
                        optimizer.kl_weight: 0 if epoch< 5 else kl_weight}
...
def eval_fetch_dict(i, epochs, min_epochs, model, optimizer):
    return {'loss VAE': optimizer.loss_VAE, 
            'loss RL': optimizer.loss_RL, 'loss V': optimizer.loss_V,
            'kl': optimizer.kl_weight}
...
def test_fetch_dict(model, optimizer):
    return {'loss VAE': optimizer.loss_VAE, 
            'loss RL': optimizer.loss_RL, 'loss V': optimizer.loss_V,
            'la': optimizer.la,"kl": optimizer.kl_weight}
...
# model
model = GraphVAEModel(data.vertexes,
                      data.bond_num_types,
                      data.atom_num_types,
