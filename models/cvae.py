...
from models import postprocess_logits,encoder_rgcn
...
class GraphVAEModel:
    ...
    def __init__(self, vertexes, edges, nodes, features, embedding_dim, encoder_units, decoder_units, variational,
        ...
        self.node_features = tf.placeholder(dtype=tf.float32, shape=(None, None,None ))
        ...
        with tf.variable_scope('encoder'):
            self.node_features_reshaped=tf.reshape(self.node_features,(-1,16,1))
            outputs = self.encoder(
                (self.adjacency_tensor, self.node_features_reshaped if with_features else None, self.node_tensor),
            ...
            self.embeddings=self.embeddings_mean+(tf.distributions.Normal(0.,1.).sample())*self.embeddings_std
            self.node_features_reshaped2=tf.reshape(self.node_features,(-1,16)) 
            self.conditional_embeddings=tf.concat([self.embeddings,self.node_features_reshaped2],axis=-1,name="conditional_latent")

        with tf.variable_scope('decoder'):
            self.edges_logits, self.nodes_logits = self.decoder(self.conditional_embeddings, decoder_units, vertexes, edges, nodes,
                                                                training=self.training, dropout_rate=0.)
    ...
    def V_x(self, inputs, units):
        with tf.variable_scope('value', reuse=tf.AUTO_REUSE):
            outputs = encoder_rgcn(inputs, units=units[:-1], training=self.training, dropout_rate=0.)
