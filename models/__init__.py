...
from utils.layers import multi_graph_convolution_layers, graph_aggregation_layer, multi_dense_layers,multi_cvae_convolution_layers
...
def encoder_cvae(inputs, units, training, dropout_rate=0.):
    graph_convolution_units, auxiliary_units = units

    with tf.variable_scope('graph_convolutions'):
        output = multi_cvae_convolution_layers(inputs, graph_convolution_units, activation=tf.nn.tanh,
                                                dropout_rate=dropout_rate, training=training)
...
def decoder_adj(inputs, units, vertexes, edges, nodes, training, dropout_rate=0.):
    ...
    with tf.variable_scope('edges_logits'):
        ...
        edges_logits = tf.layers.dropout(edges_logits, dropout_rate, training=training,name="edges_out")
    ...
    with tf.variable_scope('nodes_logits'):
        ...
        nodes_logits = tf.layers.dropout(nodes_logits, dropout_rate, training=training,name="nodes_out")
